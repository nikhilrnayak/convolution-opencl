...
int main(int argc, char** argv)
{
	...
	// Check to see if we found at least one accelerator device, otherwise return
	if (deviceIDs == NULL) {
		std::cout << "No accelerators device found" << std::endl;
		exit(-1);
	}
    ...
    for (unsigned int y = 0; y < outputSignalHeight; y++)
	{
		for (unsigned int x = 0; x < outputSignalWidth; x++)
		{
